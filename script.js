...
function setupDeleteAllFixtures() {
  ...
  if (!button) return;
  button.addEventListener("click", deleteAllFixtures);
}
...
if (!matches.length) {
  alert("ℹ️ Hakuna fixtures za kufuta.");
  return;
}

const firstConfirm = confirm(
  "⚠️ DELETE ALL FIXTURES?\n\nFixtures zote za sasa zitafutwa."
);

if (!firstConfirm) return;

const secondConfirm = confirm(
  "🚨 CONFIRM AGAIN\n\nUnataka kufuta fixtures zote? Action hii haiwezi kurudishwa."
);
...
try {
  ...
  await Promise.all(
    fixtureIds.map((fixtureId) =>
      deleteDoc(doc(db, "matches", fixtureId))
    )
  );
  ...
  alert("✅ Fixtures zote zimefutwa.");
} catch (error) {
  console.error("Delete all fixtures error:", error);
  alert("❌ Imeshindikana kufuta fixtures zote.");
}
